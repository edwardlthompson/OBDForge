...
class ExplainDtcUseCase(
    ...
    suspend fun explain(code: String, persona: PersonaMode): ExplainDtcOutcome {
        val normalized = LocalAiPolicy.normalizeCode(code) ?: return ExplainDtcOutcome.InvalidCode
        val catalogEntry = DtcCatalog.lookup(normalized)
            ...
            llmEngine.generate(prompt).getOrNull()?.takeIf { it.isNotBlank() }?.let { summary ->
                return ExplainDtcOutcome.Success(
                    DtcExplanation(
                        ...
                        severity = classification?.severity ?: catalogEntry?.severity
                            ?: dev.foss.obdforge.domain.ai.DtcSeverity.Unknown,
        ...
        val catalog = DtcCatalog.explain(normalized)
